									...
									<div class="table-responsive">
										<div style="width:1120px; height:225px; overflow-y:scroll">
											...
											<table id="sample-table-1"
												...
												<tbody>
												<c:forEach items="${listUMR }" var="l">
													<tr>
														<td><a href="#">${l.userName }</a></td>
														<td class="hidden-480">${l.mechanismName }</td>
														<td class="hidden-480">
														<c:if test="${l.right==0 }">超级管理员</c:if>
														<c:if test="${l.right==1 }">评审用户</c:if>
														<c:if test="${l.right==2 }">机构负责人</c:if>
														<c:if test="${l.right==3 }">一般用户</c:if>
														</td>

														<td>
															<div
																class="visible-md visible-lg hidden-sm hidden-xs btn-group">
																<a href="deleteUser.action?userId=${l.userId }">
