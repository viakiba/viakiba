...
//		path1 = "/";
//	} else {
//		path1 = window.location.pathname.substr(0, b) + "/";
//	}
	return a;
};
...
SWFUpload.Console.writeLine = function(d) {
	...
	try {
		...
		updateCommon.updateAlert("Exception: " + c.name + " Message: " + c.message)
	}
};
	...
	var handlers = {
		...
		onDialogClose : function(filesSelected, filesQueued, queueLength) {
			...
			if ($.inArray('onDialogClose', settings.overrideEvents) < 0) {
				if (this.queueData.filesErrored > 0) {
					updateCommon.updateAlert(this.queueData.errorMsg);
				}
			}
		...
		onUploadError : function(file, errorCode, errorMsg) {
			...
			case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
				updateCommon.updateAlert('The upload limit has been reached (' + errorMsg + ').');
